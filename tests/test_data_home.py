...
def test_registry_names_the_default_s40_head() -> None:
    reg = registry()
    assert reg["default_head"] == "aa3di_s40_seed40_head.pt"
    head = reg["files"][reg["default_head"]]
    assert head["sha256"]
    assert head["distance_cutoff"] == 0.24699
...
def test_locate_falls_back_to_data_home(
    ...
    home = tmp_path / "cache"
    home.mkdir()
    target = home / "aa3di_s40_seed40_head.pt"
    target.write_bytes(b"ok")
    monkeypatch.setenv(DATA_HOME_ENV, str(home))
    found = locate("aa3di_s40_seed40_head.pt", kind="Projection head")
...
def test_locate_missing_known_file_names_sha256(
    ...
    with pytest.raises(FileNotFoundError, match="sha256: b4e5c2f7") as exc:
        locate("aa3di_s40_seed40_head.pt", kind="Projection head")
    msg = str(exc.value)
    assert DATA_HOME_ENV in msg
    assert "aa3di_s40_seed40_head.pt" in msg
